...
## 🤖 Integração com n8n (Perguntas Direcionadas)

Quando uma ação operacional entra no status `PENDENTE`, a inteligência artificial gera automaticamente perguntas para validar a eficácia da ação com os clientes. 

Para que o n8n envie essas perguntas pelo WhatsApp, siga estes passos no seu workflow:

1. Adicione um **Node do Supabase** ou uma **Requisição HTTP** para buscar as perguntas ativas:
```sql
SELECT pergunta 
...
2. Adicione a pergunta retornada ao fluxo de mensagens enviadas ao cliente pelo WhatsApp para complementar a conversa.
Quando a ação for concluída, as perguntas serão desativadas automaticamente.
